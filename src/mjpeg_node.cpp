...
namespace pilsbot_oakd
{
        ...
        : Node("mjpeg_node", options)
    {
        ...
        bool try_gst = this->declare_parameter<bool>("use_gstreamer", true);
        ...
        // Try to initialize GStreamer only if requested and compiled with GStreamer support
    #ifdef HAVE_GSTREAMER
        if (try_gst)
        {
            // init will be attempted inside start_device (sets use_gst_)
        }
    #endif
        start_device();
    }
    ...
    void MJPEGNode::start_device()
    {
        ...
        try
        {
            std::lock_guard<std::mutex> g(gst_mutex_);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (decoder_name)
                            {
                                RCLCPP_INFO(this->get_logger(), "Selected GStreamer decoder: %s", decoder_name);
                            }
        ...
        {
            RCLCPP_WARN(this->get_logger(), "Exception while initializing GStreamer: %s", e.what());
        }
#endif
    }
    ...
    void MJPEGNode::worker_loop()
    {
        while (running_ && rclcpp::ok())
        {
            try
            {
                ...
                {
                    // Try decode via GStreamer pipeline; fallback to cv::imdecode if it fails
                    try
                    {
