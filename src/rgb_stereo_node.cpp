  ...
  int cFramerate)
{
  ...
  if (mResolution == "720p") {
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "rclcpp"), "Invalid parameter. -> monoResolution: %s", mResolution.c_str());
  ...
  monoLeft->setFps(mFramerate);
  monoRight->setResolution(monoResolution);
  monoRight->setBoardSocket(dai::CameraBoardSocket::RIGHT);
  monoRight->setFps(mFramerate);
...
int main(int argc, char ** argv)
{
  ...
  auto stereoCameraInfo = depthConverter.calibrationToCameraInfo(
    calibrationHandler,
    dai::CameraBoardSocket::RIGHT,
    monoWidth, monoHeight);
  ...
  if (usePreview) {
    rgbPreviewPublish = std::make_unique<dai::rosBridge::BridgePublisher<sensor_msgs::msg::Image,
        dai::ImgFrame>>(
      previewQueue,
      node,
